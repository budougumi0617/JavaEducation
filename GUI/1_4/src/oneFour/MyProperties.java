...
class MyProperties extends Dialog implements ActionListener, ItemListener{
	...
	public MyProperties(Clock owner) {
		...
		this.setLayout(gbl);
		this.addComp(new Label("FontType", Label.RIGHT), 0, 0, 1, 1, GridBagConstraints.EAST);
		fontType.add("Serif");
		fontType.add("SansSerif");
		this.addComp(fontType, 1, 0, 1, 1, GridBagConstraints.WEST);
		this.addComp(new Label("FontSize", Label.RIGHT), 0, 1, 1, 1, GridBagConstraints.EAST);
		fontSize.add("Small");
		fontSize.add("Middle");
		fontSize.add("Large");
		fontSize.select(1);
		this.addComp(fontSize, 1, 1, 1, 1, GridBagConstraints.WEST);
		this.addComp(new Label("FontColor", Label.RIGHT), 0, 2, 1, 1, GridBagConstraints.EAST);
		fontColor.add("Black");
		fontColor.add("White");
		fontColor.add("Blue");
		this.addComp(fontColor, 1, 2, 1, 1, GridBagConstraints.WEST);
		this.addComp(new Label("BackColor", Label.CENTER), 0, 3, 1, 1, GridBagConstraints.EAST);
		backColor.add("Black");
		backColor.add("White");
		backColor.add("Blue");
		backColor.add("Red");
	...
	void changeFontType(List obj){
		if (obj.getSelectedIndex() == 0) owner.setFontName("Serif");
		if (obj.getSelectedIndex() == 1) owner.setFontName("SansSerif");
	}
	...
	void changeBackground(List obj) {
		...
		if (obj.getSelectedIndex() == 3) owner.setBackColor("Red");
	}
	void setListDefault(){
